...
(?,?,?,?,?  ,?,?,?,?,?,?,?)
    ...
    if not os.path.isfile(json_file):
        logging.error('"' + json_file + '" file not found to pilot insertion order. Is it a folder generated by split.py ?')
        return
...
(?,?,?   ,?,?   ,?,?,?,?,?,?   ,?,?,?)
    ...
    for i in range(1, len(data)):
        ...
        toks_file = os.path.join(json_dir, doc_json.get('file') + ".csv")
        if i == 1 and not os.path.isfile(toks_file):
            logging.warning('"' + toks_file + '" file not found for lemma. Full text searching will not be available for this text.')
            do_toks = False
        if do_toks:
            toks( toks_file, doc_id)


def toks(tsv_path: str, doc_id: int):
    if not os.path.isfile(tsv_path):
        logging.error('"' + tsv_path + '" file not found for lemma. Have you processed this text with lemmatize.py?')
        return
